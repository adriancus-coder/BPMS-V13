...
function queueSpeechText(eventId, text) {
  ...
  const prev = speechBuffers.get(eventId) || {
    text: '',
    timer: null,
    startedAt: Date.now()
  };
...
app.get('/api/events/active', (req, res) => {
  ...
  if (!event) {
    return res.status(404).json({ ok: false, error: 'Nu există eveniment activ.' });
  }
...
app.post('/api/events/:id/glossary', (req, res) => {
  ...
  if (!lang) {
    return res.status(400).json({ ok: false, error: 'Limbă lipsă.' });
  }
  ...
  if (permanent) {
    db.globalMemory[`${lang.toUpperCase()}::${source}`] = target;
  }
...
app.post('/api/events/:id/audio', (req, res) => {
  ...
  if (typeof req.body.audioMuted === 'boolean') {
    event.audioMuted = req.body.audioMuted;
  }

  if (typeof req.body.audioVolume === 'number') {
    event.audioVolume = Math.max(0, Math.min(100, req.body.audioVolume));
  }

  saveDb();
  io.to(`event:${event.id}`).emit('audio_state', {
    audioMuted: event.audioMuted,
...
app.post('/api/events/:id/transcribe', upload.single('audio'), async (req, res) => {
  ...
  if (!client) {
    return res.status(400).json({ ok: false, error: 'OpenAI nu este configurat.' });
  }
...
io.on('connection', (socket) => {
  ...
  socket.on('submit_text', async ({ eventId, text }) => {
    const event = db.events[eventId];
    if (!event) {
      return socket.emit('server_error', { message: 'Eveniment inexistent.' });
    }
  ...
  socket.on('set_audio_state', ({ eventId, audioMuted, audioVolume, code }) => {
    ...
    if (typeof audioVolume === 'number') {
      event.audioVolume = Math.max(0, Math.min(100, audioVolume));
    }

    saveDb();
    io.to(`event:${eventId}`).emit('audio_state', {
      audioMuted: event.audioMuted,
      audioVolume: event.audioVolume
    });
  });
